...
respuesta llegue a **una sola base de datos** en la laptop — **sin internet en
...
## 1) Preparación (UNA sola vez, con internet, antes del evento)

Instalar las dependencias mientras **aún hay internet**:
...
> (para el Excel) y `qrcode` (para dibujar el QR). Después de esto, ya **no se
> necesita internet nunca más**.

Para verificar que todo funciona (opcional pero recomendado):
...
1. En un celular **Android**: activar **modo avión**, luego encender **WiFi** y
   el **Punto de acceso / Hotspot**. No necesita SIM ni datos. (En iPhone
   funciona si tiene SIM.) → **ese celular crea la red.**
2. Conectar **la laptop** a ese hotspot. *(La laptop es cliente; NO enciende
   ningún hotspot propio.)*
3. Conectar los **celulares de los participantes** al **mismo** hotspot.
4. Anotar el **Nombre de red (SSID)** y la **Contraseña** del hotspot: se usan
...
**Para mostrar también un QR que conecte al WiFi** (recomendado), arrancar con el
...
Dejar esa ventana **abierta** durante todo el evento y la laptop **enchufada**.
...
1. **QR de WiFi** (si se arrancó con `--ssid`): al escanearlo, el celular **se
   une a la red** sin teclear la clave.
2. **QR de la encuesta**: al escanearlo, **abre la encuesta**.

Dónde verlos:
- **Panel** (`http://localhost:8080/panel`): muestra los QR grandes y el conteo
  en vivo. Ideal para proyectarlo o ponerlo en una pantalla.
- **Imagen** `http://localhost:8080/qr.png` (encuesta) y `/wifi-qr.png` (WiFi):
  se pueden abrir e **imprimir** para pegarlas en el stand.

> **Nota:** la IP de la laptop (y por tanto el QR de la encuesta) **cambia** según
> la red a la que se conecte. Por eso lo mejor es **mostrar el panel en vivo** en
> una pantalla; si se prefiere imprimir, regenerar el QR cuando cambie la red.
...
4. Listo: la respuesta ya quedó en la laptop.
...
> Hay que elegir **"Sí, mantener conexión"**. La página abre igual. Si un celular
> insiste en desconectarse, conviene desactivar sus "datos móviles" un momento.
...
- Se ve el **conteo en vivo** de respuestas y premios (se actualiza cada 4 s).
- Botón **"Exportar Excel"** → descarga el archivo con las **49 columnas** de
  ...
  para procesarlo como de costumbre.

También se puede exportar entrando a `http://localhost:8080/export.xlsx`.

---

## 6b) Controlar la probabilidad de premio EN VIVO (todos los celulares)

La configuración de la ruleta vive en el **servidor**, así que lo que se cambie
en el admin de la laptop se aplica a **todos los celulares** en ≤15 segundos.

1. En la laptop (página servida), abrir el **engrane** → escribir el **PIN**.
2. En la tabla de premios, cambiar la columna **Peso**:
...
Ejemplo: si parece que están ganando demasiado, basta con bajar el peso de los
premios (p. ej. de 15/8 a 5/3) y en segundos todos giran con menos probabilidad.
Está protegido con el **PIN**: solo quien lo sepa puede cambiarlo.

> Nota: el **stock** (cantidad) se cuenta por dispositivo; para el control de
> "cuánta gente gana" conviene usar el **Peso**, que sí es central.
...
- **Respaldo:** conviene exportar el Excel de vez en cuando (cada hora, por
  ejemplo) y copiar `respuestas.db` a una USB. Así, aunque algo pase con la
  laptop, hay copias.
- Al recuperar internet, si se desea, se puede subir el Excel a la nube.
...
| Problema | Solución |
|---|---|
| El celular no abre la página | Verificar que esté conectado a la **WiFi del hotspot** (no a otra red). Probar escribir la URL a mano. |
| Sale "Red sin internet" | Es normal (no hay internet). Elegir **"Mantener conexión"**. |
| El QR no funciona | El servidor lista **todas las IP** de la laptop al iniciar; sirve la que empiece por la red del hotspot (`192.168.x.x` o `172.20.10.x`). El panel arma el QR con la correcta. |
| "openpyxl no instalado" | Ejecutar `pip install -r requirements.txt` (con internet, una vez). |
| No se ve el QR en la terminal | No pasa nada: usar el panel `/panel` o `/qr.png`. |
| Muchos celulares a la vez se traban | El hotspot de un celular aguanta ~8–15 equipos. Para 20–50 celulares, usar un **router WiFi** (aguanta 32–64+). La laptop se conecta a él igual. |
...
| Archivo | Qué es |
|---|---|
| `index.html` | La encuesta + ruleta (responsive: celular, tablet, PC). **Envía** cada respuesta al servidor. |
...
> **Modo kiosco:** si se abre `index.html` como archivo (doble clic, sin
> servidor), funciona como kiosco — guarda solo en ese dispositivo. El envío al
> servidor **solo** se activa cuando la página se abre desde `http://…` (es
> decir, servida por `servidor.py`).
